...
class QLearner(object):
  ...
  def __init__(
    ...
    # YOUR CODE HERE
    # 应注意： Dueling DQN和Double DQN仅仅在q_func上不同(即网络中间的结构)
    self.q1_all_ph=q_func(obs_t_float, self.num_actions, "q_estimate")
    self.q2_all_ph=q_func(obs_tp1_float, self.num_actions, "q_target")
    q_func_vars=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_estimate')
    target_q_func_vars=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_target')

    ### Dueling DQN
    q1_a_ph=tf.reduce_sum(tf.one_hot(self.act_t_ph, depth=self.num_actions)*self.q1_all_ph, axis=1)
    q2_a_tar_ph=self.rew_t_ph+gamma*(1-self.done_mask_ph)*tf.reduce_max(self.q2_all_ph, axis=1)
    self.total_error=tf.reduce_mean(huber_loss(q1_a_ph-q2_a_tar_ph))


    ### Double DQN+DUeling DQN
    q1_all_ph_=q_func(obs_tp1_float, self.num_actions,"q_estimate", reuse=True)
    action_max_ph=tf.argmax(q1_all_ph_, axis=1)
    doubleQ_action_max_ph=tf.reduce_sum(tf.one_hot(action_max_ph, depth=self.num_actions)*self.q2_all_ph, axis=1)
    doubleQ_tar_ph=self.rew_t_ph+gamma*(1-self.done_mask_ph)*doubleQ_action_max_ph
    self.total_error_doubleDQN=tf.reduce_mean(huber_loss(q1_a_ph-doubleQ_tar_ph))
    ######
    ...
    ## dqn training operation
    self.train_fn = minimize_and_clip(optimizer, self.total_error,
                 var_list=q_func_vars, clip_val=grad_norm_clipping)
    ## double dqn training operation
    self.train_fn_doubleDQN=minimize_and_clip(optimizer, self.total_error_doubleDQN,
                 var_list=q_func_vars, clip_val=grad_norm_clipping)
  ...
  def step_env(self):
    ...
    if(np.random.uniform()>self.exploration.value(self.t) and self.model_initialized):
        q_n=self.session.run(self.q1_all_ph, feed_dict={self.obs_t_ph: self.last_obs[np.newaxis,:]})
        action=np.argmax(q_n[0])
    else:
  ...
  def update_model(self):
    ...
    if (self.t > self.learning_starts and \
      ...
      if(self.double_q):
        self.session.run(self.train_fn_doubleDQN, feed_dict=input_feed)
      else:
        self.session.run(self.train_fn, feed_dict=input_feed)
